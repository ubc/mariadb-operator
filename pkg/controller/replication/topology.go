...
func (r *singleClusterTopology) ConfigurePrimary(ctx context.Context, client *sql.Client) error {
	...
	// pod's own binlog position. This runs unconditionally (not only for replicas) so it
	// also recovers a pod left with no slave entry but a polluted gtid_slave_pos from an
	// earlier switchover attempt that ran RESET SLAVE ALL and then aborted before this
	// step -- the moodle-education-stg-db field incident, where such a pod stayed wedged.
	//
	// Why align with gtid_binlog_pos rather than blanket reset to '': with
	// gtid_strict_mode the server rejects `SET @@global.gtid_slave_pos = ''` whenever the
	// binlog already contains GTIDs for any domain (Error 1948 -- "Specified value for
	// @@gtid_slave_pos contains no value for replication domain X. This conflicts with the
	// binary log which contains GTID Y"). A promoted primary's own binlog always has at
	// least its initial GTID_LIST event, so a blanket reset would fail on every healthy
	// promotion. Aligning slave_pos with the binlog pos satisfies strict_mode and is
	// semantically harmless: gtid_slave_pos is only consulted by a running slave thread,
	// and we are configuring this pod as a primary.
	...
	if gtidSlavePos != gtidBinlogPos {
		if gtidBinlogPos == "" {
			// Tolerate Error 1948 as a fallback: it can still surface here when
			// log_slave_updates=OFF (multi-cluster, PITR) and the promoted replica
			// already has binary logs. Returning it would block switchover/failover.
...
func (r *singleClusterTopology) changeMaster(ctx context.Context, mariadb *mariadbv1alpha1.MariaDB, client *sql.Client,
	...
	if resetErr := client.ResetSlave(ctx); resetErr != nil {
